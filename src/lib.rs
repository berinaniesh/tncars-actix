    ...
    pub async fn run(&self) -> std::io::Result<()> {
        ...
        return HttpServer::new(move || {
            App::new()
                ...
                .service(routes::users::get_user_id)
                .service(routes::users::get_user_username)
                .service(routes::users::get_users_posts_id)
                .service(routes::users::get_users_posts_username)
                .service(routes::follows::follow_user_by_username)
                .service(routes::follows::follow_user_by_id)
        })
        .bind(("127.0.0.1", port))?
        .run()
        .await;
